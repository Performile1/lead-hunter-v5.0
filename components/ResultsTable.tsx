      ...
      <div className="bg-white shadow-md w-full overflow-x-auto">
        <div className="min-w-[840px]">
            ...
            {paginatedData.map((lead) => {
                ...
                <div 
                    key={lead.id} 
                    className={`grid grid-cols-[40px_110px_minmax(150px,2fr)_minmax(100px,1fr)_110px_60px_minmax(150px,1.5fr)_90px] hover:bg-slate-50 transition-colors group py-3 items-center gap-2 ${isDownloaded ? 'bg-green-50/30' : ''} ${isSelected ? 'bg-black/5' : ''}`}
                    onClick={(e) => {
                ...
                >
                    ...
                    <input 
                        type="checkbox" 
                        className="rounded-sm border-slate-300 text-black focus:ring-[#2563EB] cursor-pointer"
                        checked={isSelected}
                    ...
                    <div className="min-w-0 pr-2">
                        <div className="text-xs font-bold text-black truncate" title={safeRender(lead.companyName)}>
                    ...
                    <span className={`px-1.5 py-0.5 inline-flex text-[9px] font-bold uppercase rounded-sm border justify-center w-8 ${
                            lead.segment === 'KAM' ? 'bg-black text-white border-red-800' :
                            lead.segment === 'FS' ? 'bg-black text-black border-yellow-500' :
