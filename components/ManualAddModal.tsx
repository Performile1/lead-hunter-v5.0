...
export const ManualAddModal: React.FC<ManualAddModalProps> = ({ isOpen, onClose, onAdd }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-[80] flex items-center justify-center p-4 backdrop-blur-sm animate-fadeIn">
      <div className="bg-white w-full max-w-md shadow-2xl border-t-4 border-[#D40511] relative">
        <button 
          onClick={onClose}
        ...
        <div className="bg-[#FFCC00] p-4 border-b border-slate-200">
          <h2 className="text-lg font-black italic uppercase flex items-center gap-2 text-black">
            <PlusCircle className="w-5 h-5 text-[#D40511]" />
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-xs font-bold text-slate-800 mb-1 flex items-center gap-1">
              <Building2 className="w-3 h-3 text-[#D40511]" />
              Företagsnamn (Krav)
            </label>
            <input
              type="text"
          ...
          <div className="pt-2">
             <button
              type="submit"
              className="w-full bg-[#D40511] hover:bg-[#a0040d] text-white py-2 px-4 text-sm font-bold uppercase tracking-wider shadow-sm transition-colors rounded-sm"
             >
                Lägg till i Listan
             </button>
